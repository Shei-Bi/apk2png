...
public:
    int columnType;
    CSVTable* table;
    CSVNode(LogicArrayList<StringView>* lines) {
        columnType = 0;
        table = NULL;
        load(lines);
    }
    void load(LogicArrayList<StringView>* lines) {
        for (auto it : *lines) parseLine(it, lines->size());
        // for(int i=0;i<)
    }
    void parseLine(StringView line, int totalLines) {
        int element_start = 0;
        int elements_loaded = 0;
        String something_on_stack;
        while (element_start < line.length()) {
            int element_end = line.find_first_of(',', element_start);
            if (element_end == StringView::npos) element_end = line.length();
            int has_quote = 0;
    ...
    static CSVNode* fromFile(char* name) {
        ...
        LogicArrayList<StringView> lines;
        int line_start = 0;
